...
async def exchangeForDay(phone, session, run_Time, rid):
    ...
    while cs < run_Time:
        threading.Thread(target=run_conversion_rights, args=(phone, rid, session, loop)).start()
        # threading.Thread(target=run_conversion_rights, args=(phone, rid, session, loop)).start()
        if sleepTime: 
...
async def conversionRights(phone, aid, session):
    ...
    cookies = ruishucookie.initCookie()
    if not cookies:
        print("Failed to get cookies from Ruishucookie.py")
        return

    paraV = encrypt_para(value)
    response = session.post('https://wapside.189.cn:9001/jt-sign/paradise/conversionRights', json={"para": paraV}, headers={'Cookie': cookies})
    if response:
        login = response.json()
...
async def getLevelRightsList(phone, session):
    ...
    paraV = encrypt_para(value)
    response = session.post('https://wapside.189.cn:9001/jt-sign/paradise/getLevelRightsList', json={"para": paraV}, headers={'Cookie': cookies})
    # print(f"Response from getLevelRightsList: {response.text}")  # 打印原始响应内容
    data = response.json()
    ...
    if ids:
        ...
        responseTest = session.post('https://wapside.189.cn:9001/jt-sign/paradise/getParadiseInfo', json={"para": paraV}, headers={'Cookie': cookies})
        # print(f"Response from getParadiseInfo: {responseTest.text}")  # 打印原始响应内容
        if responseTest.status_code == 200:
            try:
                response_json = responseTest.json()
                # print(f"Parsed JSON from getParadiseInfo: {response_json}")  # 打印解析后的 JSON 数据
                loginTest = response_json
                rights = loginTest.get('rights')
                # if "话费" in rights['righstName']:
                #     printn(f"{get_first_three(phone)}, {rights['righstName']}")
                if rights['rightsSize'] == 1 :
                    # printn(f"{get_first_three(phone)}, {rights['righstName']}已兑换")
                    ids.clear()
            except json.JSONDecodeError:
                print(f"{get_first_three(phone)} getParadiseInfo 返回数据不是有效的 JSON 格式: {responseTest.text}")
        else:
            print(f"{get_first_three(phone)} getParadiseInfo 请求失败，状态码: {responseTest.status_code}, 响应内容: {responseTest.text}")
    return ids
...
async def getSign(ticket, session):
    try:
        ...
        response_data = session.get('https://wapside.189.cn:9001/jt-sign/ssoHomLogin?ticket=' + ticket, headers={'Cookie': cookies}).json()
...
async def main(timeValue, isTRUE):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for phoneV in phone_list:
            ...
            ticket = await loop.run_in_executor(executor, userLoginNormal, phone, password)
            if ticket:
