...
console.log('开始等待 50 秒...');
    setTimeout(() => {
    console.log('50 秒已过，执行后续代码');
// 这里可以添加你想要在等待后执行的代码
...
async function userTask(user) {
    DoubleLog(`\n========== 账号[${user.index}]开始抢兑 ==========`)
    const taskIds = ['792556957722198016'];
    const exchangeCountid = ['3795'];
    const exchangeNeedPointsid = ['1800'];
    for(let i = 0; i < taskIds.length; i++) {
        for(let j = 0; j < 10; j++) {
            await SignTask(user, taskIds[i], exchangeCountid[i], exchangeNeedPointsid[i]);
        }
    }
    
}
// =============================================================================================================================
async function SignTask(user,pointsMallCardId,exchangeCount,exchangeNeedPoints) {
    try {
        let urlObject = {
            method: 'get',
            url: `https://channel.cheryfs.cn/archer/activity-api/pointsmall/exchangeCard?pointsMallCardId=${pointsMallCardId}&exchangeCount=${exchangeCount}&mallOrderInputVoStr=%7B%22person%22:%22%22,%22phone%22:%22%22,%22province%22:%22%22,%22city%22:%22%22,%22area%22:%22%22,%22address%22:%22%22,%22remark%22:%22%22%7D&channel=1&exchangeType=0&exchangeNeedPoints=${exchangeNeedPoints}&exchangeNeedMoney=0&cardGoodsItemIds=`,
            headers: {
