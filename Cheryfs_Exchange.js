...
//console.log('开始等待 30 秒...');
//setTimeout(() => {
//  console.log('30 秒已过，执行后续代码');
...
async function main() {
    ...
    if (queryPointsMallCardList && queryPointsMallCardList.result) {
    ...
    if (queryByPointsMallCardId && queryByPointsMallCardId.result) {
...
async function commonGet(url) {
    const proxyUrl = 'https://service.ipzan.com/core-extract?num=1&no=20250227698256492446&minute=1&pool=quality&secret=c59gk4o5ujvbui';
    const proxy = await getProxy(proxyUrl);
...
async function getProxy(proxyUrl) {
    return new Promise(resolve => {
        $.get({ url: proxyUrl }, (err, resp, data) => {
            if (err) {
                console.log('获取代理 IP 失败:', err);
                resolve(null);
            } else {
                try {
                    // 尝试解析为 JSON，如果成功说明是错误信息
                    JSON.parse(data);
                    console.log('提取代理 IP 出现错误:', data);
                    resolve(null);
                } catch {
                    // 不是 JSON 格式，说明是 IP 文本
                    const ips = data.split('\r\n');
                    if (ips.length > 0) {
                        const proxy = `http://${ips[0]}`;
                        console.log('获取到的代理 IP 地址是:', proxy);
                        resolve(proxy);
                    } else {
                        console.log('未提取到有效的代理 IP');
                        resolve(null);
                    }
                }
            }
