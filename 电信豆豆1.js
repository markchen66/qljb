...
const concurrency = 5;
const chunks = [];
for (let i = 0; i < _0x1a907b.userList.length; i += concurrency) {
    chunks.push(_0x1a907b.userList.slice(i, i + concurrency));
    await Promise.all(chunks.pop().map(user => user.userTask()));
    await _0x1a907b.wait(1000); // 每批间隔1秒
}
    }
